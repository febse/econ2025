...
Other measures of locations are the mode (the most frequent value)  and the quantiles. In general, the mode *does not make sense* only makes sense for
...
The first quartile of `cnt` (25-th percentile and 0.25 quantile are different names for the same thing) is the number of bike rentals per hour such that 
about 25 percent (one quarter) of the hours had fewer rentals.
...
The next important feature of the data is its variability. It answers the following question: how different are the customers between each other with respect to body height (for example). There are numerous ways to measure variability. To illustrate this, let us consider look at the boxplot of 
the `cnt` variable by `season`.
