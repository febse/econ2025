...
- Compute the average income and the total income and
store them into the variables `income_avg` and `income_total`, respectively. Use the `mean` and `sum` functions to calculate the values.
...
R is a case-sensitive language. This means that `Hello` and `hello` are two completely different strings. This also applies to function names. For example, `mean` and `Mean` are two different functions in R. Uncomment (remove the # character from the beginning of the second line)
and run the following code chunk to see the error message that R will produce.
...
As you have already noticed in @exr-r-vector-arithmetic, R is a vectorized language. This means that most operations are performed element-wise on vectors. In most situations this
is a convenient feature, because it allows you to write concise and readable code. However, 
you should be aware of how R handles operations on vectors of different lengths.
...
Use the `incomes` vector from the previous exercise. Create a new vector called `high_income` that holds the logical values of whether the income is greater than the average. Count
the number of elements with high income and find the proportion of high incomes.
...
Use the `incomes` vector from the previous exercise. Create a new vector called `low_income` that holds the logical values of whether the income is less than the average. Select the last element of the new vector and print it to the console (Hint: use the `length` function to count the number of elements in this vector).
...
You can think about the `pipe` operator as the word "then". For example, 
in human language the following would be the sentence.
...
Take the values in `x`, then (pipe) pass them to the `mean` function, then (pipe) take the result
of the `mean` funciton and pass it to the `abs` function (calculates the absolute value), then (pipe) pass the result from `abs` to the `sqrt` function (calculates the square root), then (pipe) round the result to two decimal places (`round`).
