...
You can insert the whole `<-` operator by pressing `Alt -` (minus). It is important to note 
that `<-` and `< -` (with space between the characters) do completely different things (see below).
:::
...
Use the `incomes` vector from the previous exercise. Create a new vector called `high_income` that holds the logical values of whether the income is greater than the average. Count the number of elements with high income and find the proportion of high incomes.
:::
