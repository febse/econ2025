...
$$
\begin{align*}
& \text{Y}_i \sim N(\mu_i, \sigma^2 = 20^2), \quad i = 1,\ldots, N = 1e6 \\
...
N <- 2e6 # Number of children in the population
...
sim_coeffs <- sim %>%
  ...
  group_map(function(d, id) {
     ...
     out["r.squared"] <- fit_summary$r.squared
...
1. What is the **expected** value of $\hat{\beta}_1?$
...
2. What is the **variance** of $\hat{\beta}_1$

```{r}

```

3. The variance of he error terms can be estimated by dividing the 
residual sum of squares by the degrees of freedom of the model
...
To get the _estimated_ variance of $\hat{\beta}_1$, replace the unknown $\sigma^2$ with its estimate $\hat{\sigma}^2$:
