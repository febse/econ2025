...
If you find the above derivation hard to follow, try to write down
the RSS for the intercept-only equation with only two observations. Then take the derivative with respect to $\hat{\beta}_0$ and set it to zero.
...
plot_ly(x = ~x, y = ~y, z = ~z1, type = "mesh3d", data = xyz1, opacity = 0.2) %>%
  add_markers(x = ~4, y = ~2, z = ~8, color = I("red"), size = 5, opacity=1) %>%
  add_trace(
    x = c(0, 4), y = c(0, 2), z = c(0, 8),
    type = 'scatter3d', mode = 'lines',
    line = list(color = 'black', width = 5)
  ) %>%
  add_trace(
    x = c(0, -10), y = c(0, -10), z = c(0, 26),
    type = 'scatter3d', mode = 'lines',
    line = list(color = 'black', width = 5)
  ) %>%
  add_trace(
    x = c(0, 10), y = c(0, 5), z = c(0, -9),
    type = 'scatter3d', mode = 'lines',
    line = list(color = 'black', width = 5)
  )
```
